      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {services.map((service) => (
            <div
              key={service.title}
              className="bg-[#0d2240] hover:bg-[#1a3a6b] transition-colors duration-200 rounded-xl p-8 flex flex-col items-center text-center group cursor-default"
            >
              <div className="mb-5">{service.icon}</div>
              <div className="w-12 border-t border-white/30 mb-5" />
              <h3 className="text-white font-bold text-sm uppercase tracking-wide leading-snug mb-2">
                {service.title}
              </h3>
              <p className="text-blue-300 text-xs leading-relaxed">{service.subtitle}</p>
            </div>
          ))}
        </div>
      </div>
